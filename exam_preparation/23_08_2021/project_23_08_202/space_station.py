...
class SpaceStation:
    ...
    def send_on_mission(self, planet_name):
        ...
        astronauts_ready_for_mission = self.astronaut_repository.filter_available_astronauts_for_mission(5, 30)

        if len(astronauts_ready_for_mission) == 0:
            raise Exception("You need at least one astronaut to explore the planet!")

        participated_astronauts = 0
        for a in astronauts_ready_for_mission:
            if len(planet.items) == 0:
                break
            while a.oxygen > 0 and len(planet.items) > 0:
